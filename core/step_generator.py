...
def _translate_description_to_hebrew(description):
    ...
    translations = {
        ...
        'Deploy row motor piston DOWN (rows operation started, motor at rightmost position)': 'פרוס בוכנת דלת סרגל עמודות למטה (פעולת עמודות התחילה, המנוע במיקום הימני ביותר)',
        'Retract row motor piston UP (rows complete, returning home)': 'החזר בוכנת דלת סרגל עמודות למעלה (פעולות עמודות הושלמו, חוזר הביתה)',
    }
